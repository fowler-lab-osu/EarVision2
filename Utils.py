...
def calculateCountMetrics(predictedCounts, actualCounts, actualTotalInclAmbig = None):                                  
    ...
    try:          
        fluorPerDiff = fluorKernelDiff / ((actualFluor + predFluor) / 2)
    except:
        fluorPerDiff = 1
    ...
    try:
        nonFluorPerDiff = nonFluorKernelDiff / ((actualNonFluor + predNonFluor) / 2)  
    except:
        nonFluorPerDiff = 1
    ...
    try:
        totalPerDiff = totalKernelDiff / ((actualTotal + predTotal) / 2)
    except:
        totalPerDiff = 1
    ...
    if(actualTransmission != 0):
        transmissionPercentageDiff =  float(
            (abs((predictedTransmission)-(actualTransmission) ) / (actualTransmission) ) * 100)
    else:
    ...
    transmissionABSDiff = abs(transmissionDiff)   

    return {"fluorKernelDiff":fluorKernelDiff, "fluorKernelABSDiff":fluorKernelABSDiff, "fluorPerDiff":fluorPerDiff, 
            "nonFluorKernelDiff":nonFluorKernelDiff, "nonFluorKernelABSDiff":nonFluorKernelABSDiff,
            "nonFluorPerDiff":nonFluorPerDiff, "totalKernelDiff":totalKernelDiff, 
            "totalKernelABSDiff":totalKernelABSDiff, "totalPerDiff":totalPerDiff, "transmissionDiff":transmissionDiff, 
            "transmissionABSDiff":transmissionABSDiff}
...
def outputPredictionAsXML(prediction, outFileName):
    ...
    for ind, label in enumerate(labels):
        ...
        centroid = ET.SubElement(obj, "centroid")
        ...
        markerX = ET.SubElement(centroid, "MarkerX")
        markerY = ET.SubElement(centroid, "MarkerY")
